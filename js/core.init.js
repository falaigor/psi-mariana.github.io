...
jQuery(document).ready(function () {
  "use strict";
  CLOE_BROOKS_STORAGE["theme_init_counter"] = 0;
  cloe_brooks_init_actions();
});

jQuery(window).on("beforeunload", function () {
  "use strict";
  // Show preloader
  //	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.cloe_brooks_profiler').css({opacity:0});	//.animate({opacity:0}, 'fast');
...
function cloe_brooks_init_actions() {
  ...
  // Hide preloader
  //	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.cloe_brooks_profiler').animate({opacity:1}, 'slow');	//.css({opacity:1});
  jQuery("#page_preloader").animate({ opacity: 0 }, 500, function () {
    jQuery(this).css({ display: "none" });
  });

  // Check for Retina display
  if (cloe_brooks_is_retina()) {
...
function cloe_brooks_ready_actions() {
  ...
  //----------------------------------------------
  if (window.cloe_brooks_skin_ready_actions) cloe_brooks_skin_ready_actions();

  // Call theme specific action (if exists)
  //----------------------------------------------
  if (window.cloe_brooks_theme_ready_actions) cloe_brooks_theme_ready_actions();
  ...
  // Prepare menus
  if (CLOE_BROOKS_STORAGE["menu_cache"]) cloe_brooks_prepare_menus();
  ...
  if (jQuery(".header_mobile").length > 0) {
    jQuery(".header_mobile .menu_main_nav_area ul#menu_main_mobile").removeAttr(
      "id"
    );
  ...
  if (CLOE_BROOKS_STORAGE["menu_slider"]) {
  ...
  if (
    ...
    CLOE_BROOKS_STORAGE["toc_menu"] != "no"
  ) {
  ...
  jQuery(
    ...
    if (jQuery(href.substr(pos)).length > 0) {
      ...
      if (now) {
        cloe_brooks_document_animate_to(href.substr(pos));
        cloe_brooks_document_set_location(pos == 0 ? loc + href : href);
        e.preventDefault();
        return false;
      }
    }
  ...
  CLOE_BROOKS_STORAGE["top_panel_height"] = 0; //Math.max(0, jQuery('.top_panel_wrap').height());
  ...
  jQuery("#viewmore_link").on("click", function (e) {
    ...
    if (
      ...
      CLOE_BROOKS_STORAGE["viewmore_busy"] = true;
      jQuery
        .post(CLOE_BROOKS_STORAGE["ajax_url"], {
        ...
        .done(function (response) {
          ...
          if (rez.error === "") {
            ...
            if (posts_container.hasClass("isotope_wrap")) {
              ...
              cloe_brooks_init_appended_isotope(posts_container, rez.filters);
            ...
            cloe_brooks_sc_init(posts_container);
            cloe_brooks_scroll_actions();
          }
        });
    }
  ...
  jQuery(".woocommerce,.woocommerce-page").on(
    ...
    function (e) {
      ...
      var mode = jQuery(this).hasClass("woocommerce_thumbs")
        ? "thumbs"
        : "list";
      jQuery.cookie("cloe_brooks_shop_mode", mode, { expires: 365, path: "/" });
  ...
  jQuery(".popup_link")
    ...
    .on("click", function (e) {
      ...
      if (popup.length === 1) {
        ...
        cloe_brooks_toggle_popup(popup);
      }
      e.preventDefault();
      return false;
    });
  ...
  jQuery(".popup_form.login_form").submit(function (e) {
    "use strict";
    var rez = cloe_brooks_login_validate(jQuery(this));
  ...
  jQuery(".popup_form.registration_form").submit(function (e) {
    "use strict";
    var rez = cloe_brooks_registration_validate(jQuery(this));
    if (!rez) e.preventDefault();
    return rez;
  });
  ...
  jQuery("form#commentform").submit(function (e) {
    "use strict";
    var rez = cloe_brooks_comments_validate(jQuery(this));
    if (!rez) e.preventDefault();
    return rez;
  });
  ...
  jQuery(".bookmarks_add").on("click", function (e) {
    ...
    var list = jQuery.cookie("cloe_brooks_bookmarks");
    var exists = false;
    if (list) {
      try {
    ...
    if (!exists) {
      var message_popup = cloe_brooks_message_dialog(
        ...
        function (btn, popup) {
          ...
          jQuery.cookie("cloe_brooks_bookmarks", JSON.stringify(list), {
            expires: 365,
            path: "/",
          });
          setTimeout(function () {
    ...
    } else cloe_brooks_message_warning(CLOE_BROOKS_STORAGE["strings"]["bookmark_exists"], CLOE_BROOKS_STORAGE["strings"]["bookmark_add"]);
    e.preventDefault();
    return false;
  });
  ...
  jQuery(".scroll_to_top").on("click", function (e) {
    "use strict";
    window.open("https://whatsa.me/5521999554084/?t=Oi", "_blank");
    return false;
  });
  ...
  if (window.cloe_brooks_sc_init_actions) cloe_brooks_sc_init_actions();

  // Init hidden elements (if exists)
  if (window.cloe_brooks_init_hidden_elements)
...
function cloe_brooks_scroll_actions() {
  ...
  // Call skin specific action (if exists)
  //----------------------------------------------
  if (window.cloe_brooks_skin_scroll_actions) cloe_brooks_skin_scroll_actions();

  // Call theme specific action (if exists)
  //----------------------------------------------
  if (window.cloe_brooks_theme_scroll_actions)
  ...
  if (CLOE_BROOKS_STORAGE["top_panel_height"] < 1) {
  ...
  if (
    !jQuery("body").hasClass("menu_mobile") &&
    CLOE_BROOKS_STORAGE["menu_fixed"]
  ) {
    var slider_height = 0;
    ...
    if (
      scroll_offset <=
      slider_height + CLOE_BROOKS_STORAGE["top_panel_height"]
    ) {
  ...
  if (jQuery(".sidebar_outer").length > 0) {
    if (CLOE_BROOKS_STORAGE["side_panel_height"] == 0)
    ...
    if (
      ...
      CLOE_BROOKS_STORAGE["side_panel_height"] + 100
    ) {
      if (jQuery(".sidebar_outer").css("position") !== "fixed") {
        jQuery(".sidebar_outer").css({
          //'top': (scroll_offset + jQuery(window).height() - CLOE_BROOKS_STORAGE['side_panel_height'] - 100) + 'px'
          position: "fixed",
          top:
            Math.min(
              0,
  ...
  jQuery("#toc .toc_item").each(function () {
    ...
    if (
      ...
      scroll_offset + CLOE_BROOKS_STORAGE["top_panel_height"] < off_next
    )
  ...
  if (window.cloe_brooks_sc_scroll_actions) cloe_brooks_sc_scroll_actions();
}

// Infinite Scroll
function cloe_brooks_infinite_scroll() {
  "use strict";
  if (CLOE_BROOKS_STORAGE["viewmore_busy"]) return;
...
function cloe_brooks_resize_actions() {
  ...
  // Call skin specific action (if exists)
  //----------------------------------------------
  if (window.cloe_brooks_skin_resize_actions) cloe_brooks_skin_resize_actions();

  // Call theme specific action (if exists)
  //----------------------------------------------
  if (window.cloe_brooks_theme_resize_actions)
  ...
  if (window.cloe_brooks_sc_resize_actions) cloe_brooks_sc_resize_actions();
}
...
function cloe_brooks_responsive_menu() {
  if (cloe_brooks_is_responsive_need(CLOE_BROOKS_STORAGE["menu_mobile"])) {
...
function cloe_brooks_is_responsive_need(max_width) {
  "use strict";
  var rez = false;
  if (max_width > 0) {
...
function cloe_brooks_video_dimensions() {
  ...
  jQuery("video.sc_video_bg").each(function () {
    ...
    if (mejs_cont.length > 0) {
      cloe_brooks_set_mejs_player_dimensions(video, w1, h1);
      mejs_cont.css({
        //'left': -l+'px',
        top: -t + "px",
      });
...
function cloe_brooks_resize_video_background() {
  ...
  if (bg.find(".mejs-container").length > 0) {
    cloe_brooks_set_mejs_player_dimensions(bg.find("video"), w1, h1);
...
function cloe_brooks_set_mejs_player_dimensions(video, w, h) {
  "use strict";
  if (mejs) {
...
// Init Superfish menu
function cloe_brooks_init_sfmenu(selector) {
  jQuery(selector)
    .show()
    .each(function () {
      if (
      ...
      jQuery(this)
        ...
        .superfish({
          ...
          onBeforeShow: function (ul) {
            ...
            if (CLOE_BROOKS_STORAGE["css_animation"]) {
...
function cloe_brooks_show_current_menu_item(menu, button) {
  "use strict";
...
function cloe_brooks_init_isotope() {
  ...
  jQuery(".isotope_wrap:not(.inited)").each(function () {
    "use strict";
    all_images_complete =
      all_images_complete && cloe_brooks_check_images_complete(jQuery(this));
  });
  ...
  jQuery(".isotope_wrap:not(.inited)").each(function () {
    ...
    // Init shortcodes
    cloe_brooks_sc_init(isotope_container);
...
function cloe_brooks_init_appended_isotope(posts_container, filters) {
  "use strict";

  if (posts_container.parents(".sc_scroll_horizontal").length > 0) return;

  if (
    !cloe_brooks_check_images_complete(posts_container) &&
    CLOE_BROOKS_STORAGE["isotope_init_counter"]++ < 30
  ) {
    setTimeout(function () {
      cloe_brooks_init_appended_isotope(posts_container, filters);
    }, 200);
    return;
  }
  // Add filters
...
function cloe_brooks_init_post_formats() {
  ...
  // Call theme specific action (if exists)
  if (window.cloe_brooks_theme_init_post_formats)
  ...
  // Popup init
  if (CLOE_BROOKS_STORAGE["popup_engine"] == "pretty") {
  ...
  } else if (CLOE_BROOKS_STORAGE["popup_engine"] == "magnific") {
    jQuery("a[href$='jpg'],a[href$='jpeg'],a[href$='png'],a[href$='gif']").attr(
      "rel",
      "magnific"
    );
  ...
  if (jQuery(".post_counters_likes:not(.inited)").length > 0) {
    jQuery(".post_counters_likes:not(.inited)")
      ...
      .on("click", function (e) {
        ...
        var cookie_likes = cloe_brooks_get_cookie("cloe_brooks_likes");
          ...
          .done(function (response) {
            ...
            if (rez.error === "") {
              ...
              button
                ...
                .html(likes);
              cloe_brooks_set_cookie("cloe_brooks_likes", cookie_likes, 365);
            } else {
              cloe_brooks_message_warning(
                CLOE_BROOKS_STORAGE["strings"]["error_like"]
              );
            }
          });
        e.preventDefault();
...
function cloe_brooks_init_media_elements(cont) {
  if (
    CLOE_BROOKS_STORAGE["media_elements_enabled"] &&
    ...
    if (window.mejs) {
      ...
      setTimeout(function () {
        cloe_brooks_init_media_elements(cont);
      }, 400);
  }
}
...
function cloe_brooks_show_system_message() {
  if (
    ...
    if (CLOE_BROOKS_STORAGE["system_message"]["status"] == "success")
      cloe_brooks_message_success(
        CLOE_BROOKS_STORAGE["system_message"]["message"],
        CLOE_BROOKS_STORAGE["system_message"]["header"]
      );
    else if (CLOE_BROOKS_STORAGE["system_message"]["status"] == "info")
      cloe_brooks_message_info(
        CLOE_BROOKS_STORAGE["system_message"]["message"],
        CLOE_BROOKS_STORAGE["system_message"]["header"]
      );
    else if (
      CLOE_BROOKS_STORAGE["system_message"]["status"] == "error" ||
      CLOE_BROOKS_STORAGE["system_message"]["status"] == "warning"
    )
      cloe_brooks_message_warning(
        CLOE_BROOKS_STORAGE["system_message"]["message"],
        CLOE_BROOKS_STORAGE["system_message"]["header"]
...
// Toggle popups
function cloe_brooks_toggle_popup(popup) {
  if (popup.css("display") != "none") cloe_brooks_hide_popup(popup);
  else cloe_brooks_show_popup(popup);
}

// Show popups
function cloe_brooks_show_popup(popup) {
...
// Hide popups
function cloe_brooks_hide_popup(popup) {
  if (popup.css("display") != "none") {
    if (CLOE_BROOKS_STORAGE["css_animation"])
      popup
        .removeClass(
...
// Comments form
function cloe_brooks_comments_validate(form) {
  "use strict";
  form.find("input").removeClass("error_fields_class");
  var rules = {
    error_message_text: CLOE_BROOKS_STORAGE["strings"]["error_global"], // Global error message text (if don't write in checked field)
  ...
  var error = cloe_brooks_form_validate(form, rules);
  return !error;
}

// Login form
function cloe_brooks_login_validate(form) {
  "use strict";
  form.find("input").removeClass("error_fields_class");
  var error = cloe_brooks_form_validate(form, {
      ...
      .done(function (response) {
        ...
        if (rez.error === "") {
          ...
          result_box
            ...
            .html(
              CLOE_BROOKS_STORAGE["strings"]["login_failed"] +
                "<br>" +
                rez.error
            );
        }
...
// Registration form
function cloe_brooks_registration_validate(form) {
  "use strict";
  form.find("input").removeClass("error_fields_class");
  var error = cloe_brooks_form_validate(form, {
    error_message_show: true,
